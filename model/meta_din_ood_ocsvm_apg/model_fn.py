...
@model("meta_din_ood_ocsvm_apg", MetaType.ModelBuilder)
class DeepInterestNetwork(nn.Module):
    ...
    def forward(self, features, pred=False, train_ood_threshold=0.99):
        ...
        # # hist_embed_copy = deepcopy(hist_embed)
        # # hist_embed_copy = hist_embed_copy.cpu().detach().numpy()
        # # for hist_embed_item, target_embed_item in tqdm(zip(hist_embed.cpu().detach().numpy(),
        # #                                                    target_embed.cpu().detach().numpy())):
        # # for hist_embed_item in tqdm(hist_embed.cpu().detach().numpy()):
        # for hist_embed_item in hist_embed.cpu().detach().numpy():
        # # for trigger_embed_item, hist_embed_item in tqdm(zip(trigger_embed.cpu().detach().numpy(),
        # #                                                     hist_embed.cpu().detach().numpy())):
        #     lof_result = clf.fit_predict(hist_embed_item)
        #     # lof_result = clf.fit_predict(torch.cat(hist_embed_item, target_embed_item))
        #     # lof_result = clf.fit_predict(torch.cat(trigger_embed_item, hist_embed_item))
        #     # print(lof_result)
        #     if lof_result[-1] == -1:
        #         ood_pred.append(0)
        #         # print(lof_result)
        ...
        # # ood_pred = torch.Tensor(np.array(ood_pred)).view(-1, 1)
        # # print(ood_pred.size())
