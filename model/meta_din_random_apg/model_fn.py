...
@model("meta_din_random_apg", MetaType.ModelBuilder)
class DeepInterestNetwork(nn.Module):
    ...
    # def forward(self, features, pred=False, rate=20, sample_user_set=None):
    def forward(self, features, pred=False, rate=20, sample_user_tensor=None):
        # Encode target item
        # B * D
        # print("-" * 50)
        # print(features)
        ...
        if not pred:
            ...
            user_id_tensor = features[consts.FIELD_USER_ID]
            request_index = torch.where(torch.sum(torch.where(user_id_tensor.view(-1, 1) == sample_user_tensor.view(-1).repeat(user_id_tensor.size()[0], 1), 1, 0), dim=-1) == 1, True, False).view(-1)
        ...
        if pred:
            ...
            # print("=" * 50)
            # print(sample_user_tensor.view(-1).repeat(user_id_tensor.size()[0], 1).size())
            # print(user_id_tensor.view(-1, 1).size())
