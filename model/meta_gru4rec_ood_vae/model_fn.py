...
@model("meta_gru4rec_ood_vae", MetaType.ModelBuilder)
class GRU4Rec(nn.Module):
    ...
    def forward(self, features, pred=False, train_ood_threshold=0.99):
        ...
        user_embedding, trigger_embed_ = self._meta_classifier_param_list(user_state, trigger_embed,
                                         user_state.size()[0],
                                         trigger_seq_length)
        user_embedding = self._encoder(user_embedding)
        next_item_embedding = self._decoder(user_embedding)

        return next_item_embedding
